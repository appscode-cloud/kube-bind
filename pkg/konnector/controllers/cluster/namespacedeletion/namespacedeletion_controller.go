...
func NewController(
	...
	c := &controller{
		...
		getServiceNamespace: func(ns, name string) (*kubebindv1alpha1.APIServiceNamespace, error) {
			...
			return providerInfo.BindInformer.KubeBind().V1alpha1().APIServiceNamespaces().Lister().APIServiceNamespaces(ns).Get(name)
		},
		deleteServiceNamespace: func(ctx context.Context, ns, name string) error {
			...
			return provider.BindClient.KubeBindV1alpha1().APIServiceNamespaces(ns).Delete(ctx, name, metav1.DeleteOptions{})
		},
	}
...
func (c *controller) enqueueNamespace(logger klog.Logger, obj interface{}) {
	...
	ns, _, err := cache.SplitMetaNamespaceKey(nsKey)
	if err != nil {
		runtime.HandleError(err)
		return
	}
	...
	key := fmt.Sprintf("%s/%s", provider.Namespace, nsKey)
