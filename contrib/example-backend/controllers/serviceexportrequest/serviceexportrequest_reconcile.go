...
type reconciler struct {
	informerScope          kubewarev1alpha1.Scope
	clusterScopedIsolation kubewarev1alpha1.Isolation
...
func (r *reconciler) reconcile(ctx context.Context, req *kubewarev1alpha1.APIServiceExportRequest) error {
	var errs []error
...
func (r *reconciler) ensureExports(ctx context.Context, req *kubewarev1alpha1.APIServiceExportRequest) error {
	logger := klog.FromContext(ctx)

	if req.Status.Phase == kubewarev1alpha1.APIServiceExportRequestPhasePending {
		failure := false
		for _, res := range req.Spec.Resources {
			...
			if apierrors.IsNotFound(err) {
				conditions.MarkFalse(
					req,
					kubewarev1alpha1.APIServiceExportRequestConditionExportsReady,
					"CRDNotFound",
			...
			export := &kubewarev1alpha1.APIServiceExport{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						kubewarev1alpha1.SourceSpecHashAnnotationKey: hash,
					},
				},
		...
		if !failure {
			conditions.MarkTrue(req, kubewarev1alpha1.APIServiceExportRequestConditionExportsReady)
			req.Status.Phase = kubewarev1alpha1.APIServiceExportRequestPhaseSucceeded
			return nil
		}

		if time.Since(req.CreationTimestamp.Time) > time.Minute {
			req.Status.Phase = kubewarev1alpha1.APIServiceExportRequestPhaseFailed
			req.Status.TerminalMessage = conditions.GetMessage(req, kubewarev1alpha1.APIServiceExportRequestConditionExportsReady)
		}
