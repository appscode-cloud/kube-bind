...
func (r *reconciler) reconcile(ctx context.Context, export *kubewarev1alpha1.APIServiceExport) error {
	var errs []error
...
func (r *reconciler) ensureSchema(ctx context.Context, export *kubewarev1alpha1.APIServiceExport) (specChanged bool, err error) {
	...
	expected, err := kubewarehelpers.CRDToServiceExport(crd)
	if err != nil {
		conditions.MarkFalse(
			export,
			kubewarev1alpha1.APIServiceExportConditionProviderInSync,
	...
	if hash := kubewarehelpers.APIServiceExportCRDSpecHash(expected); export.Annotations[kubewarev1alpha1.SourceSpecHashAnnotationKey] != hash {
		...
		export.Annotations[kubewarev1alpha1.SourceSpecHashAnnotationKey] = hash
		return true, nil
	}

	conditions.MarkTrue(export, kubewarev1alpha1.APIServiceExportConditionProviderInSync)
